...
from app.models.database import SessionLocal, User, Conversation, Message, Tenant, Document
from app.models.schemas import Token, UserCreate, UserResponse, ConversationResponse, MessageResponse, LoginRequest
...
def register(user_in: UserCreate, db: Session = Depends(get_db)):
    ...
    tenant = Tenant(name=user_in.full_name or "Default", slug=user_in.email.split("@")[0])
    db.add(tenant)
    db.flush()
...
@router.post("/auth/conversations", response_model=ConversationResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/auth/conversations/{conversation_id}/messages", response_model=list[MessageResponse])
...
@router.delete("/auth/conversations/{conversation_id}", status_code=status.HTTP_204_NO_CONTENT)
...
def list_tenant_documents(db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    docs = db.query(Document).filter(Document.tenant_id == current_user.tenant_id).all()
    return [{"id": d.id, "filename": d.filename, "uploaded_by": d.uploaded_by, "created_at": d.created_at} for d in docs]
